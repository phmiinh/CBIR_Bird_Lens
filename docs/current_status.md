...
Cập nhật: `2026-05-09` theo múi giờ Asia/Saigon
...
- Phase 5/6 đã chạy retrieval experiments cho `50` queries x `9` experiments x `top-5`.
- Phase 7 hiện có species-proxy và auto-visual-proxy evaluation.
...
| Fusion có thể bị tổng weight `1.20` | Hiện tại không còn bug này: code và DB đều có tổng weight `1.00`; `fusion = 0.80` handcrafted + `0.20` CNN. | Đã thêm runtime guard và per-query percentile score calibration để tránh cộng raw score khác thang đo. |
...
| Manual relevance | Vẫn là blocker chính. SQLite hiện có `species_proxy` và `auto_visual_proxy`; template manual còn trống. | Cần điền label, import, rerun Phase 7/8 trước khi viết kết quả cuối. |
...
| Phase 6 - Experiments | hoàn tất phần chạy, chưa hoàn tất manual labels | `50` queries, `450` runs, `2250` top-k rows |
| Phase 7 - Evaluation | một phần | có species-proxy và auto-visual-proxy outputs, thiếu manual outputs |
...
| `query_features` | `350` |
| `retrieval_runs` | `450` |
| `retrieval_results` | `2250` |
| `experiments` | `9` |
| `relevance_judgments` | `51215` rows: `50000` species-proxy + `1215` auto-visual-proxy |
...
- file này có `1215` dòng query-candidate cần gán nhãn
- toàn bộ `relevance_grade` đang trống
- SQLite hiện có `judgment_source = 'species_proxy'` và `judgment_source = 'auto_visual_proxy'`
- chưa có `outputs/eval/manual_per_query.csv`
- chưa có `outputs/eval/manual_summary.json`

Vì sao là `1215` dòng, không phải chỉ `250` dòng:

- hiện có kết quả từ tất cả experiment configurations
- volume gốc là `50 queries x 9 experiments x 5 = 2250` rows
- template đã deduplicate các cặp query-candidate trùng nhau, còn `1215` rows cần annotate
...
| `cnn_only` | `0.232` |
| `fusion` | `0.040` |
| `calibrated_handcrafted` | `0.020` |
| `color_layout_only` | `0.020` |
| `texture_shape_only` | `0.004` |
| `ablation_no_regional_color` | `0.028` |
| `ablation_no_explicit_shape` | `0.032` |
| `ablation_no_shape` | `0.028` |

Auto-visual-proxy cũng đã có để sanity-check pipeline, nhưng không dùng làm ground truth cuối vì proxy này được sinh từ descriptor similarity. Các số species-proxy chỉ nên trình bày như baseline phụ. Không nên dùng species-proxy để kết luận chất lượng CBIR cuối cùng, vì bài này là retrieval theo độ tương đồng thị giác chứ không phải species classification.
...
- `experiment_summaries.csv` mới có species-proxy và auto-visual-proxy metrics
